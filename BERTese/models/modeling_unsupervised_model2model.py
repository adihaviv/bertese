...
from transformers.modeling_auto import AutoModel, AutoModelWithLMHead
logger = logging.getLogger(__name__)


class PreTrainedEncoderUnsupervisedDecoder(nn.Module):
    r"""
    ...
    def __init__(self, encoder, decoder, encoder_hidden_size,decoder_hidden_size):
        super().__init__()
        self.encoder = encoder
        self.decoder = decoder
        self.proj_layer = nn.Linear(encoder_hidden_size, decoder_hidden_size)
    ...
    def from_pretrained(
        ...
        Params:
            encoder_pretrained_model_name_or_path: information necessary to initiate the encoder. Either:
                ...
                - a string with the `shortcut name` of a pre-trained model to load from cache or download, e.g.: ``lower_bert-base-uncased``.
                - a string with the `identifier name` of a pre-trained model that was user-uploaded to our S3, e.g.: ``dbmdz/lower_bert-base-german-cased``.
        ...
        Examples::
            ...
            model = PreTrainedEncoderDecoder.from_pretained('lower_bert-base-uncased', 'lower_bert-base-uncased') # initialize Bert2Bert
        """
        ...
        model = cls(encoder, decoder, encoder.config.hidden_size, decoder.config.hidden_size)
    ...
    def forward(self, encoder_input_ids, **kwargs):
        ...
        decoder_input = self.proj_layer(encoder_hidden_states)
        decoder_outputs = self.decoder(inputs_embeds=decoder_input, **kwargs_decoder)

        return decoder_outputs + encoder_outputs


class UnsupervisedModel2Model(PreTrainedEncoderUnsupervisedDecoder):
    r"""
        ...
        by creating it beforehand as follows
            ...
            model = Model2Model.from_pretrained('lower_bert-base-uncased', decoder_model=decoder)
    """
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        if (
                "lower_bert" not in pretrained_model_name_or_path
